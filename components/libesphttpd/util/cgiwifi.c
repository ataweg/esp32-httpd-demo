...
static void ICACHE_FLASH_ATTR reassTimerCb( void *arg )
{
   ...
   // !!! todo ( AWe ): start a timer and then retry, limit retries
   ...
   if( opmode != STATION_MODE )
   {
      ...
      os_timer_arm( &resetTimer, 30000, 0 ); // time out after 30 secs of trying to connect
   }
}
...
CgiStatus ICACHE_FLASH_ATTR cgiWiFiConnect( HttpdConnData *connData )
{
   ...
   httpdFindArg( connData->post.buff, "password", password, sizeof( password ) );
...
CgiStatus ICACHE_FLASH_ATTR cgiWiFiSetMode( HttpdConnData *connData )
{
   ...
   if( len != 0 )
   {
      ESP_LOGI( TAG, "cgiWifiSetMode: %d", atoi( buff ) );
#ifndef DEMO_MODE
...
CgiStatus ICACHE_FLASH_ATTR tplWlan( HttpdConnData *connData, char *token, void **arg )
{
   ...
   {
      opmode = wifi_get_opmode();
      if( opmode == STATION_MODE )   strcpy( buff, "Station" );
      if( opmode == SOFTAP_MODE )    strcpy( buff, "SoftAP" );
      if( opmode == STATIONAP_MODE ) strcpy( buff, "Station + SoftAP" );
   }
   ...
   else if( strcmp( token, "WiFiApWarn" ) == 0 )
   {
      ...
      {
         strcpy( buff, "<b>Can't scan in this mode.</b> Click <a href=\"wifi/setmode.cgi?mode=1\">here</a> to go to Station mode."
                       " or <a href=\"wifi/setmode.cgi?mode=3\">here</a> to go to Station + SoftAP mode." );
      }
      else if( opmode == STATIONAP_MODE )
      {
         strcpy( buff, "Click <a href=\"wifi/setmode.cgi?mode=1\">here</a> to go to Station mode"
                       " or <a href=\"wifi/setmode.cgi?mode=2\">here</a> to go to SoftAP mode." );
      }
      else  // STATION_MODE
      {
         strcpy( buff, "Click <a href=\"wifi/setmode.cgi?mode=3\">here</a> to go to Station + SoftAP mode"
                       " or <a href=\"wifi/setmode.cgi?mode=2\">here</a> to go to SoftAP mode." );
      }
   }
