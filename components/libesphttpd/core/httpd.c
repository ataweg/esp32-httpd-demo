...
static void ICACHE_FLASH_ATTR httpdRetireConn( HttpdInstance *pInstance, HttpdConnData *connData )
{
   ...
   {
      free( connData->post.buff );
      connData->post.buff = NULL;
   }
...
void ICACHE_FLASH_ATTR httpdStartResponse( HttpdConnData *connData, int code )
{
   ...
   l = os_snprintf( buff, sizeof( buff ), "HTTP/1.%d %d %s\r\nServer: %s\r\n%s",
...
{
   // if( connData->isConnectionClosed ) return 0;
   ...
   if( connData->priv->flags & HFL_CHUNKED && connData->priv->flags & HFL_SENDINGBODY && connData->priv->chunkHdr == NULL )
   {
      ...
      assert( connData->priv->sendBuffLen <= HTTPD_MAX_SENDBUFF_LEN );
   }
...
#define httpdSend_orDie( connData, data, len ) do { if ( !httpdSend( ( connData ), ( data ), ( len ) ) ) return false; } while ( 0 )

// encode for HTML. returns true = success, or false
bool ICACHE_FLASH_ATTR httpdSend_html( HttpdConnData *connData, const char *data, int len )
{
   int start = 0, end = 0;
   char c;
   ...
   for( end = 0; end < len; end++ )
   {
      ...
      if( c == '"' || c == '\'' || c == '<' || c == '>' )
      {
         if( start < end ) httpdSend_orDie( connData, data + start, end - start );
         start = end + 1;
      }

      if( c == '"' ) httpdSend_orDie( connData, "&#34;", 5 );
      else if( c == '\'' ) httpdSend_orDie( connData, "&#39;", 5 );
      else if( c == '<' ) httpdSend_orDie( connData, "&lt;", 4 );
      else if( c == '>' ) httpdSend_orDie( connData, "&gt;", 4 );
   }
...
// encode for HTML. returns true = success, or false
bool ICACHE_FLASH_ATTR httpdSend_js( HttpdConnData *connData, const char *data, int len )
{
   int start = 0, end = 0;
   char c;
   if( len < 0 ) len = ( int ) strlen( data );
   ...
   for( end = 0; end < len; end++ )
   {
      ...
      if( c == '"' ) httpdSend_orDie( connData, "\\\"", 2 );
      else if( c == '\'' ) httpdSend_orDie( connData, "\\'", 2 );
      else if( c == '\\' ) httpdSend_orDie( connData, "\\\\", 2 );
      else if( c == '<' ) httpdSend_orDie( connData, "\\u003C", 6 );
      else if( c == '>' ) httpdSend_orDie( connData, "\\u003E", 6 );
      else if( c == '\n' ) httpdSend_orDie( connData, "\\n", 2 );
      else if( c == '\r' ) httpdSend_orDie( connData, "\\r", 2 );
   }

   if( start < end )
      httpdSend_orDie( connData, data + start, end - start );
   return true;
}
...
static void ICACHE_FLASH_ATTR httpdProcessRequest( HttpdInstance *pInstance, HttpdConnData *connData )
{
   ...
   while( 1 )
   {
      ...
      if( pInstance->builtInUrls[i].url == NULL )
      {
         ...
         ESP_LOGI( TAG, "%s not found. 404", connData->url );
...
static CallbackStatus ICACHE_FLASH_ATTR httpdParseHeader( char *h, HttpdConnData *connData )
{
   ...
   if( firstLine )
   {
      ...
      *e = 0; // terminate url part
      ...
      if( strcasecmp( e, "HTTP/1.1" ) == 0 ) connData->priv->flags |= HFL_HTTP11 | HFL_CHUNKED;

      ESP_LOGI( TAG, "URL = %s", connData->url );
   ...
   else if( strncasecmp( h, "Content-Length:", 15 ) == 0 )
   {
      ...
      connData->post.buff = ( char* )malloc( bufferSize );
   ...
   else
   {
      ...
      for( x = 0; x < len; x++ )
      {
         ...
         else if( connData->post.len != 0 )
         {
            ...
            connData->post.buff[connData->post.buffLen++] = data[x];
            ...
            if( connData->post.buffLen >= connData->post.buffSize || connData->post.received == connData->post.len )
            {
               ...
               connData->post.buff[connData->post.buffLen] = 0; // zero-terminate, in case the cgi handler knows it can use strings
               // Process the data
               if( connData->cgi )
               {
...
CallbackStatus ICACHE_FLASH_ATTR httpdDisconCb( HttpdInstance *pInstance, HttpdConnData *connData )
{
   ...
   ESP_LOGI( TAG, "httpdDisconCb: Connection closed: %s", S( connData->url ) );
...
{
   ESP_LOGI( TAG, "httpdConnectCb: create a new connection" );
